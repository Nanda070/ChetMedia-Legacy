...
# Подключаем Album
from app.database import get_db, User, Media, Album
    ...
    while True:
        ...
        try:
            ...
            for m in expired_solos:
                # Проверяем, не использует ли этот же файл кто-то другой (дедупликация)
                count = db.query(Media).filter(Media.file_path == m.file_path).count()
                if count == 1 and m.file_path and Path(m.file_path).exists():
                    os.remove(m.file_path) # Удаляем физически только если это последняя ссылка
                db.delete(m)
            ...
            for album in expired_albums:
                ...
                db.delete(album) # Каскадно удалит и записи фоток из этого альбома
                deleted_albums_count += 1
                
            if deleted_solos_count > 0 or deleted_albums_count > 0:
                db.commit()
                print(f"🗑️ [КЛИНЕР] Очистка завершена. Удалено одиночных файлов: {deleted_solos_count}, альбомов: {deleted_albums_count}")
            else:
...
async def lifespan(app: FastAPI):
    # Код, который выполнится ПРИ ЗАПУСКЕ сервера
    asyncio.create_task(cleanup_expired_media_loop())
    yield
    # Код, который выполнится ПРИ ВЫКЛЮЧЕНИИ сервера (если нужен)
...
async def callback(code: str, db: Session = Depends(get_db)):
    ...
    if not db_user:
        db_user = User(id=discord_id, username=user_info["username"])
        db.add(db_user)
        db.commit()
...
def read_users_me(current_user: User = Depends(get_current_user)):
    return {"message": "Ты в системе!", "discord_id": current_user.id, "username": current_user.username}
    ...
    try:
        ...
        if media:
            if success:
                ...
                now = datetime.now(timezone.utc)
                media.expires_at = now + timedelta(days=30) if duration <= 15 * 60 else now + timedelta(days=14)
...
# ОДИНОЧНАЯ ЗАГРУЗКА (РАБОТАЕТ КАК РАНЬШЕ)
@app.post("/upload")
async def upload_media(
    ...
    if existing_file and existing_file.file_path:
        ...
        final_filename = f"{file_id}{file_extension}" if is_image else f"{file_id}.mp4"
    ...
    new_media = Media(
        id=file_id, user_id=current_user.id, album_id=None,
    ...
    db.add(new_media)
    db.commit()
    # Было: "url": f"http://127.0.0.1:8000/v/{file_id}"
    return {"status": "success", "file_id": file_id, "url": f"{BASE_URL}/v/{file_id}"}

# --- НОВЫЙ ЭНДПОИНТ: МАССОВАЯ ЗАГРУЗКА АЛЬБОМА (ФОТО) ---
...
async def upload_album(
    ...
    # Броня: проверяем, что перетащили ТОЛЬКО фотографии
    for file in files:
        if not file.content_type.startswith("image/"):
            raise HTTPException(status_code=400, detail="Массовая загрузка поддерживает только фото (формат Альбома)!")

    album_id = uuid.uuid4().hex[:8]
    now = datetime.now(timezone.utc)
    expires_at = now + timedelta(days=14) # Альбом сгорает через 14 дней

    # 1. Создаем запись альбома
    db_album = Album(id=album_id, user_id=current_user.id, expires_at=expires_at)
    db.add(db_album)

    # 2. Сохраняем фотографии пачкой
    for file in files:
        ...
        if existing_file and existing_file.file_path:
            # Дедупликация работает и внутри альбомов!
            os.remove(final_path)
            final_path = Path(existing_file.file_path)

        new_photo = Media(
            id=media_id, user_id=current_user.id, album_id=album_id, # Цепляем к альбому
    ...
    db.commit()
    # Было: "url": f"http://127.0.0.1:8000/a/{album_id}"
...
async def get_media_file(file_id: str, db: Session = Depends(get_db)):
    media = db.query(Media).filter(Media.id == file_id).first()
    if not media or not media.file_path: raise HTTPException(status_code=404, detail="Файл не найден")
    if media.status != "ready": raise HTTPException(status_code=400, detail="Файл обрабатывается")
...
async def view_album(request: Request, album_id: str, db: Session = Depends(get_db)):
    ...
    if not album:
        raise HTTPException(status_code=404, detail="Альбом не найден или удален")
    ...
    return templates.TemplateResponse(
        ...
        context={
            "request": request, 
            "photos": album.media, 
            "expires_at": album.expires_at.strftime("%Y-%m-%d %H:%M")
        }
    )
...
def get_my_media(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for m in media_files:
        result.append({
            ...
            "status": m.status, "duration": m.duration, "album_id": m.album_id, # Отдаем ID альбома фронтенду
            "expires_at": m.expires_at.strftime("%Y-%m-%d %H:%M"),
            # Было: "url": f"http://127.0.0.1:8000/v/{m.id}" ...
...
def delete_my_file(file_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Если файл привязан к альбому — удаляем ВЕСЬ АЛЬБОМ, чтобы не оставлять сирот
    if media.album_id:
